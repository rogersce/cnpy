...
cnpy::NpyArray load_the_npy_file(FILE* fp) {
    ...
    int float32_size = 4;
    char* fp16_buffer = NULL;
    bool use_fp16 = (word_size == 2);
    if (use_fp16) {
        fp16_buffer = new char[size * word_size];
        arr.data = new char[size * float32_size]; // restore weights as float32
    }
    else {
    ...
    size_t nread = 0;
    if (use_fp16) {
        nread = fread(fp16_buffer, word_size, size, fp);
    }
    else {
        nread = fread(arr.data, word_size, size, fp);
    }
    if(nread != size)
        throw std::runtime_error("load_the_npy_file: failed fread");
    if (use_fp16) {
        for (int i=0; i<size; i++) {
            uint16_t h = *(uint16_t*)(fp16_buffer + word_size * i);
            *(uint32_t *)(arr.data + float32_size * i) = ((h&0x8000)<<16) | (((h&0x7c00)+0x1C000)<<13) | ((h&0x03FF)<<13);
        }
        delete fp16_buffer;
        arr.word_size = float32_size; // Deceive arr so that it thinks it's reading a is normal float32 array
    }
    return arr;
}
...
cnpy::NpyArray cnpy::npz_load(std::string fname, std::string varname) {
    ...
    while(1) {
        ...
        //std::cout << "Scanning header of variable: " << vname << std::endl;
